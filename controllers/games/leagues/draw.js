    ...
    try {
        do {
            ...
            /*            if (chunk.length !== 2) {
                            ...
                            error.statusCode = ResponseStatus.MethodNotAllowed; //edit this; determine a suitable custom status code
                            throw error;
                        } */
            chunky.push(chunk);
        } while (list.length);
...
module.exports = (_mode, _type, contesters) => {
    ...
    if (_mode === 0) { // kickout cup mode
        if (contesters.length % 2) { // if number of contesters if an odd number
            ...
            error.statusCode = ResponseStatus.MethodNotAllowed; //edit, creat new code for this?
            throw error;
        }
        try {
